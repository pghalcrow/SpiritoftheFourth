...
describe('AdminComponent', () => {
  ...
  it('opens a user roles help modal from the users panel', () => {
    ...
    expect(modal.textContent).toContain('Permissions');
    expect(modal.textContent).toContain('Manage events');
    expect(modal.textContent).toContain('Manage users');
    expect(modal.textContent).not.toContain('Create users');
    expect(modal.textContent).not.toContain('Remove users');
    expect(modal.textContent).not.toContain('View users');
    expect(modal.querySelector('[data-testid="permission-superAdmin-manageEvents"]')?.textContent?.trim()).toBe('✓');
    expect(modal.querySelector('[data-testid="permission-admin-manageEvents"]')?.textContent?.trim()).toBe('—');
    expect(modal.querySelector('[data-testid="permission-admin-manageUsers"]')?.textContent?.trim()).toBe('✓');
    expect(modal.querySelector('[data-testid="permission-viewer-manageUsers"]')?.textContent?.trim()).toBe('—');
    expect(modal.querySelector('[data-testid="permission-viewer-viewSubmissions"]')?.textContent?.trim()).toBe('✓');
  ...
  it('clears the admin session and redirects when the admin users token is expired', () => {
    spyOn(console, 'error');
    const router = TestBed.inject(Router) as jasmine.SpyObj<Router>;
    sessionStorage.setItem('adminToken', 'expired-token');
    sessionStorage.setItem('adminRole', 'superAdmin');
    sessionStorage.setItem('adminEmail', 'super@example.com');
    cmsService.getAdminUsers.and.returnValue(throwError(() => ({ status: 401 })));
    ...
    expect(sessionStorage.getItem('adminEmail')).toBeNull();
    expect(router.navigate).toHaveBeenCalledWith(['/sign-in']);
    expect(fixture.nativeElement.querySelector('.admin-modal')?.textContent).not.toContain('Users unavailable');
  });
